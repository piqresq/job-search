...
async function getSetting(db: D1Database, userId: string, key: string): Promise<string | null> {
  const row = await db
...
export async function getCvExtractionFromDb(
  db: D1Database,
  userId: string,
): Promise<CvExtractionRow> {
  const [text, html, at, sanitized] = await Promise.all([
    getSetting(db, userId, CV_SOURCE_TEXT_KEY),
    getSetting(db, userId, CV_SOURCE_HTML_KEY),
    getSetting(db, userId, CV_UPLOADED_AT_UNIX_KEY),
    getSetting(db, userId, CV_SANITIZED_TEXT_KEY),
  ]);
...
export async function setCvExtractionCache(
  ...
  await db.batch([
    db
      ...
      .bind(userId, CV_SOURCE_TEXT_KEY, opts.text),
    db
      .prepare(
        `INSERT INTO app_settings (user_id, key, value) VALUES (?, ?, ?)
         ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value`,
      )
      .bind(userId, CV_SOURCE_HTML_KEY, opts.html),
    db
      .prepare(
        `INSERT INTO app_settings (user_id, key, value) VALUES (?, ?, ?)
         ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value`,
      )
      .bind(userId, CV_UPLOADED_AT_UNIX_KEY, String(opts.uploadedAtUnix)),
    db
      .prepare(
        `INSERT INTO app_settings (user_id, key, value) VALUES (?, ?, ?)
         ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value`,
      )
      .bind(userId, CV_SANITIZED_TEXT_KEY, sanitizedText),
  ]);
}
